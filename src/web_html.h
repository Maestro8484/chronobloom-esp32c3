...
static const char INDEX_P2[] PROGMEM =
  ...
  "<div class='row'><div><label>Outer ring brightness (%)</label><input id='outerRingBrightness' type='number' min='0' max='100' style='width:70px'></div></div>\n"
  "<div class='row'><div><label>Middle face scale (0-255)</label><input id='middleFaceScale' type='number' min='0' max='255' style='width:70px'></div><div><label>Inner face scale (0-255)</label><input id='innerFaceScale' type='number' min='0' max='255' style='width:70px'></div></div>\n"
  "<div class='row'><div><label>Contrast preset</label><select id='contrastPreset' onchange='applyContrastPreset(this.value)'><option value=''>Custom</option><option value='current'>Current defaults</option><option value='soft'>Soft contrast</option><option value='clear'>Clear contrast</option><option value='high'>High contrast</option><option value='balanced'>Balanced glow</option></select></div></div>\n"
  ...
  "<div class='row'><button class='primary' onclick='saveSettings()'>Save display</button><button onclick='resetToDefaults()' style='color:#c0392b'>Reset all to defaults</button></div>\n"
  "</div>\n"
  ...
  "    <option value='1'>Slow comet</option>\n"
  "    <option value='2'>Dual orbit</option>\n"
  "    <option value='3'>Bloom ripple</option>\n"
  ...
  "    <option value='0'>Off</option>\n"
  "    <option value='1'>Unfurl</option>\n"
  "    <option value='2'>Three comets</option>\n"
  ...
  "  <option value='2'>Galaxy spin</option>\n"
  "  <option value='3'>Supernova</option>\n"
  "  <option value='4'>Comet relay</option>\n"
  "  <option value='5'>Deep breath</option>\n"
  ...
  "</div>\n"
  "<div class='row'>\n"
  "  <div><label>Preview slot</label><select id='stylePreviewType'>\n"
  "    <option value='quarter'>Quarter (current selection)</option>\n"
  "    <option value='halfhour'>Half-hour (current selection)</option>\n"
  "    <option value='hour'>Hour (current selection)</option>\n"
  "    <option value='reminder'>Reminder (current selection)</option>\n"
  "  </select></div>\n"
  ...
  "  <option value='6'>Gentle pulse</option>\n"
  "  <option value='7'>Orbiting orb</option>\n"
  "  <option value='8'>Ripple in</option>\n"
  "  <option value='9'>Heartbeat</option>\n"
  "  <option value='10'>Slow bloom</option>\n"
  ...
  "</div>\n"
  "<div class='panel'><h2>&#127912; Color Theme</h2>\n"
  "<p class='sub' style='font-size:12px;color:#92a0b5'>Export or import ring color and level settings as a portable JSON file. Themes do not include brightness or animation settings.</p>\n"
  "<div class='row'>\n"
  "  <button onclick='exportTheme()'>&#8681; Export theme</button>\n"
  "  <label style='display:inline-flex;align-items:center;gap:8px;margin:0;cursor:pointer;background:#203146;border:1px solid #42546d;border-radius:6px;padding:8px 14px;color:var(--text)'>\n"
  "    &#8679; Import theme\n"
  "    <input type='file' id='themeImportFile' accept='.json' style='display:none' onchange='importTheme(this.files[0])'>\n"
  "  </label>\n"
  "</div>\n"
  "<div id='themeStatus' style='font-size:12px;color:#92a0b5;margin-top:8px;min-height:16px'></div>\n"
  "</div>\n"
...
static const char INDEX_P3[] PROGMEM =
  ...
  "function bindLive(){for(const k of ['outerMarkerLevel','outerFillerLevel','secondsLevel','minutesLevel','hoursLevel','centerLevel']){const out=qs(k+'Out');const upd=()=>{out.value=qs(k).value;draw()};qs(k).oninput=upd;upd()}for(const k of ['outerMarkerColor','outerFillerColor','secondsColor','minutesColor','hoursColor','centerColor','previewMode'])qs(k).oninput=draw;for(const k of ['secondTrail','progressSeconds'])qs(k).oninput=()=>{settings[k]=qs(k).checked;draw()};for(const k of ['animationBrightness','trailLength']){const out=qs(k+'Out');const upd=()=>{out.value=qs(k).value};qs(k).oninput=upd;upd()}for(const k of ['outerRingBrightness','middleFaceScale','innerFaceScale'])qs(k).oninput=()=>{qs('contrastPreset').value=''}}\n"
  ...
  "async function resetToDefaults(){if(!confirm('Reset ALL settings to factory defaults?'))return;await fetch('/settings/reset',{method:'POST'});loadSettings()}\n"
  "async function previewAnim(type,modeId){const mode=qs(modeId).value;if(!mode||mode==='0')return;const p=new URLSearchParams();p.set('animationPalette',qs('animationPalette').value);p.set('animationSpeed',qs('animationSpeed').value);p.set('animationBrightness',qs('animationBrightness').value);p.set('trailLength',qs('trailLength').value);p.set('reminderPalette',qs('reminderPalette').value);p.set('silent','1');await fetch('/settings',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:p.toString()});fetch('/previewAnimation',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'type='+type+'&mode='+mode})}\n"
  "async function previewStyleAnim(){const p=new URLSearchParams();p.set('animationPalette',qs('animationPalette').value);p.set('animationSpeed',qs('animationSpeed').value);p.set('animationBrightness',qs('animationBrightness').value);p.set('trailLength',qs('trailLength').value);p.set('reminderPalette',qs('reminderPalette').value);p.set('silent','1');await fetch('/settings',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:p.toString()});const slotType=qs('stylePreviewType').value;const modeIds={quarter:'quarterAnimation',halfhour:'halfHourAnimation',hour:'hourAnimation',reminder:'focusReminder_animation'};const modeId=modeIds[slotType];const mode=modeId?qs(modeId).value:'0';if(!mode||mode==='0')return;fetch('/previewAnimation',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'type='+slotType+'&mode='+mode})}\n"
  "async function saveAnimStyle(){const p=new URLSearchParams();p.set('animationPalette',qs('animationPalette').value);p.set('animationSpeed',qs('animationSpeed').value);p.set('animationBrightness',qs('animationBrightness').value);p.set('trailLength',qs('trailLength').value);p.set('reminderPalette',qs('reminderPalette').value);p.set('silent','1');await fetch('/settings',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:p.toString()});loadSettings()}\n"
  ...
  "const CONTRAST_PRESETS={current:{outerRingBrightness:90,middleFaceScale:50,innerFaceScale:50},soft:{outerRingBrightness:35,middleFaceScale:50,innerFaceScale:50},clear:{outerRingBrightness:24,middleFaceScale:28,innerFaceScale:28},high:{outerRingBrightness:9,middleFaceScale:14,innerFaceScale:14},balanced:{outerRingBrightness:22,middleFaceScale:55,innerFaceScale:55}};\n"
  "function applyContrastPreset(val){if(!val)return;const p=CONTRAST_PRESETS[val];if(!p)return;qs('outerRingBrightness').value=p.outerRingBrightness;qs('middleFaceScale').value=p.middleFaceScale;qs('innerFaceScale').value=p.innerFaceScale;saveSettings()}\n"
  "function updateContrastPreset(){const orb=Number(qs('outerRingBrightness').value),mfs=Number(qs('middleFaceScale').value),ifs=Number(qs('innerFaceScale').value);let matched='';for(const[k,p] of Object.entries(CONTRAST_PRESETS)){if(p.outerRingBrightness===orb&&p.middleFaceScale===mfs&&p.innerFaceScale===ifs){matched=k;break}}qs('contrastPreset').value=matched}\n"
  "makeClock();loadSettings();refresh();loadNet();setInterval(refresh,1000);setInterval(draw,90);\n"
  "function exportTheme(){const fields=['outerMarkerColor','outerFillerColor','secondsColor','minutesColor','hoursColor','centerColor'];const levels=['outerMarkerLevel','outerFillerLevel','secondsLevel','minutesLevel','hoursLevel','centerLevel'];const theme={version:1,outerRingBrightness:Number(qs('outerRingBrightness').value)};for(const k of fields)theme[k]=qs(k).value;for(const k of levels)theme[k]=Number(qs(k).value);const blob=new Blob([JSON.stringify(theme,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='chronobloom-theme.json';a.click();URL.revokeObjectURL(a.href);qs('themeStatus').textContent='Theme exported.'}\n"
  "function importTheme(file){if(!file)return;const reader=new FileReader();reader.onload=e=>{try{const theme=JSON.parse(e.target.result);const fields=['outerMarkerColor','outerFillerColor','secondsColor','minutesColor','hoursColor','centerColor'];const levels=['outerMarkerLevel','outerFillerLevel','secondsLevel','minutesLevel','hoursLevel','centerLevel'];for(const k of fields)if(theme[k])qs(k).value=theme[k];for(const k of levels)if(theme[k]!=null)qs(k).value=theme[k];if(theme.outerRingBrightness!=null)qs('outerRingBrightness').value=theme.outerRingBrightness;draw();saveSettings();qs('themeStatus').textContent='Theme imported and saved.'}catch(err){qs('themeStatus').textContent='Import failed: invalid JSON.'}};reader.readAsText(file)}\n"
  "</script></body></html>";
